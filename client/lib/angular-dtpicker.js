angular.module('datetimepicker-directive',[]).directive('datetimepicker', function($parse){
	return function(scope, element, attrs, controller){
		var ngModel = $parse(attrs.dtpModel)
		...
		//		 scope.$apply()
		// }
		var picker = element.appendDtpicker({
			dateFormat: 'YYYY-MM-DD hh:mm',
			angular: {
				parse: $parse,
				scope: scope,
				ng_model_name: attrs.dtpModel,
				ng_model: ngModel
			}
		})
		// console.dir(picker)
		// scope.$watch(attrs.dtpModel,function(new_value){
 ...
 (function($) {
	...
	var outputToInputObject = function($picker, no_scope_apply) {
		var date = getPickedDate($picker);
		...
		if(!no_scope_apply){
			var ng = $picker.data("angular")
			ng.ng_model.assign(ng.scope, date)
			if(!ng.scope.$$phase)
				ng.scope.$apply()
		}
		//
	...
	var init = function($obj, opt) {
		...
		ng.scope.$watch(ng.ng_model_name,function(new_value){
			...
			//alert(ng.ng_model_name+":"+new_value)
		})
		//
